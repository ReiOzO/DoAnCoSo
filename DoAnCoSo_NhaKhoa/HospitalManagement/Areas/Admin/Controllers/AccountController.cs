...
namespace HospitalManagement.Areas.Admin.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        [HttpPost]
        public JsonResult Delete(Guid id, int role)
        {
            try
            {
                using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
                {
                    var account = workScope.Accounts.FirstOrDefault(x => x.Id == id && !x.IsDeleted && x.Role == role);
                    if (account != null)
                    {
                        // Thực hiện đánh dấu xóa
                        account.IsDeleted = true;
                        workScope.Complete(); // Lưu thay đổi vào cơ sở dữ liệu

                        return Json(new { status = true, mess = "Xóa thành công tài khoản" });
                    }
                    else
                    {
                        return Json(new { status = false, mess = "Không tồn tại tài khoản để xóa" });
                    }
                }
            }
            catch (Exception ex)
            {
                // Đảm bảo bắt và xử lý các ngoại lệ
                return Json(new { status = false, mess = "Xảy ra lỗi trong quá trình xóa: " + ex.Message });
            }
        }
    }
}